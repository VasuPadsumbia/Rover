...
#address = 0x80
#broker_address = "172.22.18.198"  # Replace with the actual broker address
#broker_port = 1883  # Replace with the actual broker port
#topic = "Laser_Scanner"  # Replace with the desired topic
...
class ObstacleAvoidance():
    ...
    def __init__(self,threshold):
        'self.port = port ' # Change this to the port where your scanner is connected
   ...
   def scan(self):
        ...
        # Parse data into polar coordinates
        angles = []
        distances = []

        for i in range(0, len(data), 3):
            angle = int(data[i])  # Angle in degrees
            distance = int(data[i+2])  # Distance in cm
    ...
    '''def avoid_obstacles(self):
        distance,angle = SICK.calc_distances()
        'self.threshold =threshold'

        # Check for obstacles
        for i in range(len(angle)):
            if distance[i] < self.threshold and 85 <= angle[i] <= 95:
                print('Stopped:Obstacle')
                #RobotMove.stop(0x80)# Implement logic to change the rover's direction here
...
#class Obstacle:
class SICK():
    def __init__(self, port, debug=False, password="SICK_PLS"):
        self._debug = debug
    ...
    def calc_distances(self):
        ...
        for i in range(0, 361):
            ...
            coords = np.append(coords, np.array([[distance,angle]]), axis=0)

        self.polar = coords
        #print('polar;',Sdistances)
    ...
    def make_image(self):
        img = np.zeros(((MAX_DIST + 50), (MAX_DIST + 50), 3), np.uint8)
        for c in self.polar:
                cv2.circle(img,(int(c[0] + img.shape[1]/2),int(img.shape[0] - 20 - c[1])),2,(0,255*c[2]/MAX_DIST,255-255*c[2]/MAX_DIST),1)
                cv2.line(img, (int(img.shape[1] / 2), int(img.shape[0] / 2)),(int(c[0] + img.shape[1] / 2), int(img.shape[0] / 2 - c[1])),(0, 255 * (c[1] + 90) / 90, (255 - 255 * (c[1] + 90) / 90)))
        self.image = img
